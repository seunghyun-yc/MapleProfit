...
export async function ledgerRequest(options: RequestInit = {}): Promise<Response> {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      if (typeof input.date !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(input.date) || !Number.isFinite(Date.parse(input.date)) || new Date(input.date).toISOString().slice(0, 10) !== input.date || ![3, 5].includes(input.feePercent) || Object.entries(limits).some(([key, max]) => !Number.isSafeInteger(input[key]) || input[key] < 0 || input[key] > max)) throw new Error('날짜와 0 이상의 정수를 입력하세요.');
    }
